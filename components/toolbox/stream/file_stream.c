...
static bool file_stream_delete_and_insert(
    ...
    do {
        ...
        const size_t current_position = stream_tell(stream);
        const size_t file_size = stream_size(stream);
        const size_t size_to_delete = MIN(delete_size, file_size - current_position);
        const size_t size_to_copy_before = current_position;
        const size_t size_to_copy_after = file_size - current_position - size_to_delete;

        if(!stream_rewind(stream)) break;
        if(stream_copy(stream, scratch_stream, size_to_copy_before) != size_to_copy_before) break;

        if(write_callback && !write_callback(scratch_stream, ctx)) break;
        const size_t new_position = stream_tell(scratch_stream);
        ...
        const size_t new_file_size = stream_size(scratch_stream);

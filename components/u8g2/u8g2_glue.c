...
uint8_t u8x8_hw_spi_esp32(u8x8_t* u8x8, uint8_t msg, uint8_t arg_int, void* arg_ptr) {
    ...
    UNUSED(arg_int);
    UNUSED(arg_ptr);
    /* Stub: no real SPI operations on ESP32 port yet */
    switch(msg) {
...
void u8x8_d_st756x_set_contrast(u8x8_t* u8x8, int8_t contrast_offset) {
    /* On ESP32, default to ERC contrast since we don't have furi_hal_version */
    uint8_t contrast = CONTRAST_ERC;
...
uint8_t u8x8_d_st756x_flipper(u8x8_t* u8x8, uint8_t msg, uint8_t arg_int, void* arg_ptr) {
    ...
    if(u8x8_d_st756x_common(u8x8, msg, arg_int, arg_ptr) == 0) {
        ...
        case U8X8_MSG_DISPLAY_INIT:
            ...
            /* On ESP32, default to ERC display init parameters */

...
static void archive_file_browser_set_path(
    ...
    if(!browser->worker_running) {
        ...
        file_browser_worker_start(browser->worker);
...
static bool archive_is_dir_exists(FuriString* path) {
    ...
    if(furi_string_equal(path, STORAGE_EXT_PATH_PREFIX)) {
        FS_Error sd_err = storage_sd_status(storage);
        state = sd_err == FSE_OK;
        esp_rom_printf("[ARCH] sd_status=%d (0=OK) for '%s' -> %d\r\n", sd_err, furi_string_get_cstr(path), state);
    } else if(storage_common_stat(storage, furi_string_get_cstr(path), &file_info) == FSE_OK) {
        state = file_info_is_dir(&file_info);
        esp_rom_printf("[ARCH] stat('%s') is_dir=%d\r\n", furi_string_get_cstr(path), state);
    } else {
        esp_rom_printf("[ARCH] stat('%s') FAILED\r\n", furi_string_get_cstr(path));
    }
    furi_record_close(RECORD_STORAGE);
    return state;
}
...
void archive_switch_tab(ArchiveBrowserView* browser, InputKey key) {
    ...
    esp_rom_printf("\r\n[ARCH] switch_tab=%d path='%s'\r\n", tab, furi_string_get_cstr(browser->path));
    bool tab_empty = true;
    if(tab == ArchiveTabFavorites) {
        if(archive_favorites_count(browser) > 0) {
            tab_empty = false;
        }
        esp_rom_printf("[ARCH] favorites count>0: %d\r\n", !tab_empty);
    } else if(furi_string_start_with_str(browser->path, "/app:")) {
        ...
        esp_rom_printf("[ARCH] app tab empty=%d\r\n", tab_empty);
    } else {
    ...
    esp_rom_printf("[ARCH] tab_empty=%d tab=%d\r\n", tab_empty, tab);
    if((tab_empty) && (tab != ArchiveTabBrowser)) {
        esp_rom_printf("[ARCH] skipping empty tab %d\r\n", tab);

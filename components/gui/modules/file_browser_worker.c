...
static void browser_worker_trace(const char* step) {
    esp_rom_printf(
        "\r\n[FBW] %s free=%u\r\n",
        step,
...
static int32_t browser_worker(void* context) {
    ...
    furi_check(browser);
    browser_worker_trace("thread_start");
    ...
    browser_worker_trace("thread_end");
    FURI_LOG_D(TAG, "End");
    return 0;
}
...
BrowserWorker* file_browser_worker_alloc(
    ...
    bool skip_assets,
    bool hide_dot_files) {
    BrowserWorker* browser = calloc(1, sizeof(BrowserWorker));
    furi_check(browser);
...
void file_browser_worker_start(BrowserWorker* browser) {
    ...
    esp_rom_printf("\r\n[FBW] start stack=%u\r\n", (unsigned)BROWSER_WORKER_STACK_SIZE);
}

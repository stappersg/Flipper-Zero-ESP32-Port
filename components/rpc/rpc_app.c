/**
 * @file rpc_app.c
 * RPC App subsystem — ESP32 port
 *
 * Based on STM32 rpc_app.c. Adapted includes for ESP32 component layout.
 */
...
static void rpc_system_app_send_error_response(
    ...
    const char* status_str = status == PB_CommandStatus_ERROR_APP_NOT_RUNNING ? "APP_NOT_RUNNING" :
                                                                                "UNKNOWN";
    FURI_LOG_E(TAG, "%s: %s, id %lu, status: %d", name, status_str, (unsigned long)command_id, status);
...
static void rpc_system_app_start_process(const PB_Main* request, void* context) {
    ...
    FURI_LOG_D(TAG, "StartProcess: id %lu", (unsigned long)request->command_id);
    ...
    if(app_name) {
        ...
        if(app_args && strcmp(app_args, "RPC") == 0) {
            ...
            snprintf(app_args_temp, RPC_SYSTEM_APP_TEMP_ARGS_SIZE, "RPC %08lX", (uint32_t)(uintptr_t)rpc_app);
    ...
    FURI_LOG_D(TAG, "StartProcess: response id %lu, result %d", (unsigned long)request->command_id, result);
...
static void rpc_system_app_exit_request(const PB_Main* request, void* context) {
    ...
    if(rpc_app->callback) {
        FURI_LOG_D(TAG, "ExitRequest: id %lu", (unsigned long)request->command_id);
...
static void rpc_system_app_load_file(const PB_Main* request, void* context) {
    ...
    if(rpc_app->callback) {
        FURI_LOG_D(TAG, "LoadFile: id %lu", (unsigned long)request->command_id);

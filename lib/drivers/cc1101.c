...
CC1101Status cc1101_strobe(const FuriHalSpiBusHandle* handle, uint8_t strobe) {
    ...
    if(!cc1101_spi_trx(handle, tx, rx, 1)) {
        return cc1101_status_from_raw(0xFF);
    }

    return cc1101_status_from_raw(rx[0]);
}
...
CC1101Status cc1101_write_reg(const FuriHalSpiBusHandle* handle, uint8_t reg, uint8_t data) {
    ...
    if(!cc1101_spi_trx(handle, tx, rx, 2)) {
        return cc1101_status_from_raw(0xFF);
    }

    return cc1101_status_from_raw(rx[1]);
}
...
bool cc1101_wait_status_state(
    ...
    uint32_t timeout_us) {
    bool result = false;
    CC1101Status status = cc1101_status_from_raw(0xFF);
    ...
    while(!furi_hal_cortex_timer_is_expired(timer)) {
        ...
        if(cc1101_status_state(status) == state) {
            result = true;
            break;
        }
    }
...
void cc1101_set_pa_table(const FuriHalSpiBusHandle* handle, const uint8_t value[8]) {
    ...
    cc1101_spi_trx(handle, tx, rx, sizeof(rx));
}

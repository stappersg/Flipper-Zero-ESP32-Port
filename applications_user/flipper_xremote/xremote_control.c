...
#include "views/xremote_profile_view.h"
...
XRemoteApp* xremote_control_alloc(XRemoteAppContext* app_ctx) {
    ...
    /*
     * A learned TV, sound system, fan, or heater opens directly into its
     * matching profile remote. General and Custom keep the classic menu.
     */
    xremote_profile_load(
        infrared_remote_get_path(buttons->remote), &buttons->profile);

    if(buttons->profile != XRemoteDeviceProfileGeneral &&
       buttons->profile != XRemoteDeviceProfileCustom) {
        xremote_app_view_alloc2(
            app, XRemoteViewIRProfile, xremote_profile_view_alloc, buttons);
        xremote_app_view_set_previous_callback(app, xremote_control_submenu_exit_callback);
    }

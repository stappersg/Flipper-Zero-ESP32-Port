...
static uint8_t xremote_learn_button_count(XRemoteLearnContext* learn_ctx) {
    uint8_t count = xremote_profile_button_count(learn_ctx->profile);
    return count ? count : XREMOTE_BUTTON_COUNT;
}

const char* xremote_learn_get_curr_button_name(XRemoteLearnContext* learn_ctx) {
    xremote_app_assert(learn_ctx, NULL);
    const char* name = xremote_profile_button_name(learn_ctx->profile, learn_ctx->current_button);
    return name ? name : xremote_button_get_name(learn_ctx->current_button);
}
...
static uint32_t xremote_learn_text_input_exit_callback(void* context) {
    ...
    if(learn_ctx->current_button >= xremote_learn_button_count(learn_ctx))
        learn_ctx->current_button = xremote_learn_button_count(learn_ctx) - 1;
...
static void xremote_learn_text_input_callback(void* context) {
    ...
    if(infrared_remote_get_button_count(learn_ctx->ir_remote) &&
       learn_ctx->text_store[0] != '\0') {
        ...
        xremote_profile_store(output_file, learn_ctx->profile);
...
static bool xremote_learn_custom_event_callback(void* context, uint32_t event) {
    ...
    learn_ctx->profile = XRemoteDeviceProfileGeneral;
        }
...
XRemoteApp* xremote_learn_alloc_profile(XRemoteAppContext* app_ctx, XRemoteDeviceProfile profile) {
...
XRemoteApp* xremote_learn_alloc(XRemoteAppContext* app_ctx) {
    return xremote_learn_alloc_profile(app_ctx, XRemoteDeviceProfileGeneral);
}

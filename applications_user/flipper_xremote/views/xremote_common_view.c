...
XRemoteView*
    ...
    /* T-Embed rotary wheel always scrolls as Up/Down in either orientation. */
    view_set_input_mode(remote_view->view, ViewInputModeUpDown);
...
InfraredRemoteButton* xremote_view_get_button_by_name(XRemoteView* rview, const char* name) {
    /* A standard .ir file may not contain every XRemote command. Missing commands
       are normal and must not crash the app. */
    if(!rview || !name || !rview->context || !rview->app_ctx) return NULL;

    XRemoteAppSettings* settings = rview->app_ctx->app_settings;
    XRemoteAppButtons* buttons = (XRemoteAppButtons*)rview->context;
    if(!settings || !buttons || !buttons->remote) return NULL;
...
bool xremote_view_press_button(XRemoteView* rview, InfraredRemoteButton* button) {
    /* Do nothing for an unmapped/missing button instead of dereferencing NULL. */
    if(!rview || !rview->app_ctx || !rview->app_ctx->app_settings || !button) return false;

    InfraredSignal* signal = infrared_remote_button_get_signal(button);
    if(!signal) return false;

    dolphin_deed(DolphinDeedIrSend);
    infrared_signal_transmit_times(signal, rview->app_ctx->app_settings->repeat_count);
...
void xremote_view_model_select(XRemoteViewModel* model, uint8_t selection) {
    if(!model) return;
    model->selected = selection;
    model->ok_pressed = selection == 0;
    model->up_pressed = selection == 1;
    model->down_pressed = selection == 2;
    model->left_pressed = selection == 3;
    model->right_pressed = selection == 4;
    model->back_pressed = selection == 5;
    model->hold = false;
}

void xremote_view_model_move_selection(XRemoteViewModel* model, int direction, uint8_t count) {
    if(!model || count == 0) return;
    int next = (int)model->selected + direction;
    while(next < 0) next += count;
    next %= count;
    xremote_view_model_select(model, (uint8_t)next);
}
...
void xremote_canvas_draw_exit_footer(Canvas* canvas, ViewOrientation orient, const char* text) {
    ...
    if(orient == ViewOrientationVertical) {
        xremote_canvas_draw_icon(canvas, 6, height - 4, XRemoteIconBack);
        elements_multiline_text_aligned(canvas, 12, height, AlignLeft, AlignBottom, text);
    } else {
        uint8_t x = strncmp(text, "Hold", 4) ? (width - 57) : (width - 52);
        xremote_canvas_draw_icon(canvas, x, height - 4, XRemoteIconBack);
        elements_multiline_text_aligned(canvas, width, height, AlignRight, AlignBottom, text);
    }
}

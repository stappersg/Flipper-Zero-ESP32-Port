...
XRemoteAppButtons* xremote_app_buttons_alloc() {
    ...
    buttons->profile = XRemoteDeviceProfileGeneral;
...
/*
 * Always use the submenu API here.
 * It rotates the View and also updates the submenu's own vertical/horizontal
 * layout state. Vertical gets compact rows; horizontal keeps normal rows.
 */
    submenu_set_orientation(app->submenu, settings->orientation);

    /* Keep the T-Embed rotary wheel as Up/Down even in vertical orientation. */
    view_set_input_mode(view, ViewInputModeUpDown);

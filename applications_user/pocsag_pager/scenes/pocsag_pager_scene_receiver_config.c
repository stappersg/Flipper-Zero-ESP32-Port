...
const char* const pocsag_hopping_text[HOPPING_COUNT] = {
    "OFF",
    "ON",
};
const uint32_t pocsag_hopping_value[HOPPING_COUNT] = {
    PCSGHopperStateOFF,
...
static void pocsag_pager_scene_receiver_config_set_hopping_running(VariableItem* item) {
    ...
    if(pocsag_hopping_value[index] == PCSGHopperStateOFF) {
        char text_buf[10] = {0};
        snprintf(
            text_buf,
            sizeof(text_buf),
    ...
    app->txrx->hopper_state = pocsag_hopping_value[index];
}
...
void pocsag_pager_scene_receiver_config_on_enter(void* context) {
    ...
    value_index = pocsag_pager_scene_receiver_config_hopper_value_index(
        app->txrx->hopper_state, pocsag_hopping_value, HOPPING_COUNT, app);
    variable_item_set_current_value_index(item, value_index);

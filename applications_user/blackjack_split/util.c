...
void save_settings(Settings settings) {
    ...
    FURI_LOG_D(APP_NAME, "Saving fresh config");

    /*
     * Rebuild the complete config file instead of updating individual
     * fields in-place. The old update path crashes on this port.
     */
    if(storage_common_stat(storage, CONFIG_FILE_PATH, NULL) == FSE_OK) {
        storage_common_remove(storage, CONFIG_FILE_PATH);
    }
    ...
    if(flipper_format_file_open_new(file, CONFIG_FILE_PATH)) {
        ...
        FURI_LOG_E(APP_NAME, "Could not create settings file");
    }

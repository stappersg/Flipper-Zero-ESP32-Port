...
FontInfo* totp_font_info_alloc() {
    ...
    // ESP32 port: heap is NOT zero-initialized (unlike STM32). Without this,
    // 'name' holds garbage and totp_font_info_free() does free(garbage) ->
    // heap corruption. STM32's zeroed heap masked this upstream bug.
    font_info->name = NULL;
    return font_info;
}

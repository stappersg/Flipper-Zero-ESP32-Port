...
static const uint8_t* get_device_uid() {
    // ESP32 port: STM32's UID64_BASE memory-mapped device UID doesn't exist.
    // furi_hal_version_uid() returns the equivalent per-device unique id.
    return furi_hal_version_uid();
}

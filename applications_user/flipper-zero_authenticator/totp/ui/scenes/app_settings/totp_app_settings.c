...
typedef struct {
    ...
    // OK->edit->rotate->OK: when true, encoder rotation changes the selected
    // control's value instead of moving between controls.
    bool editing;
} SceneState;
...
void totp_scene_app_settings_render(Canvas* const canvas, const PluginState* plugin_state) {
    ...
    if(scene_state->selected_control < FontSelect) {
        ...
        ui_control_select_render(
            ...
            scene_state->editing && scene_state->selected_control == HoursInput);
        ...
        ui_control_select_render(
            ...
            scene_state->editing && scene_state->selected_control == MinutesInput);
        ...
        ui_control_select_render(
            ...
            scene_state->editing && scene_state->selected_control == FontSelect);
        ...
        ui_control_select_render(
            ...
            scene_state->editing && scene_state->selected_control == SoundSwitch);
        ...
        ui_control_select_render(
            ...
            scene_state->editing && scene_state->selected_control == VibroSwitch);
        ...
        if(scene_state->selected_control <= BadKeyboardLayoutSelect) {
            ...
            scene_state->editing && scene_state->selected_control == AutomationSwitch);
        } else {
            group_offset += 18;
        }
        ...
        if(scene_state->selected_control <= AutomationDelaySelect ||
           (scene_state->automation_method & AutomationMethodBadBt) == 0) {
            ...
            scene_state->editing && scene_state->selected_control == BadKeyboardLayoutSelect);
#ifdef TOTP_BADBT_AUTOMATION_ENABLED
        ...
        ui_control_select_render(
            ...
            scene_state->editing && scene_state->selected_control == AutomationDelaySelect);
        ...
        if((scene_state->automation_method & AutomationMethodBadBt) != 0) {
            ...
            scene_state->editing && scene_state->selected_control == BadBtProfileSelect);
        }
#endif
    } else {
        canvas_set_font(canvas, FontPrimary);
        ...
        ui_control_select_render(
            ...
            scene_state->editing && scene_state->selected_control == SplitTokenIntoGroupsSelect);
...
// Change the currently selected control's value by one step in `direction`
// (+1 = encoder CW/next, -1 = encoder CCW/previous). Used in edit mode.
static void change_selected_value(SceneState* scene_state, int8_t direction) {
    switch(scene_state->selected_control) {
    case HoursInput:
        totp_roll_value_int8_t(
            &scene_state->tz_offset_hours, direction, -12, 14, RollOverflowBehaviorStop);
        break;
    case MinutesInput:
        totp_roll_value_uint8_t(
            &scene_state->tz_offset_minutes, direction * 15, 0, 45, RollOverflowBehaviorRoll);
        break;
    case FontSelect:
        totp_roll_value_uint8_t(
    ...
    case AutomationDelaySelect:
        ...
        break;
#ifdef TOTP_BADBT_AUTOMATION_ENABLED
    case BadBtProfileSelect:
        totp_roll_value_uint8_t(
            &scene_state->badbt_profile_index, direction, 0, UINT8_MAX, RollOverflowBehaviorRoll);
        break;
#endif
    case SplitTokenIntoGroupsSelect:
        totp_roll_value_uint8_t(
            &scene_state->split_token_into_groups,
            direction,
            0,
...
// Whether OK on this control enters edit mode (rotate-to-change). Multi-value
// selects do; boolean switches (Sound/Vibro) toggle on OK directly; Confirm
// saves.
static bool control_is_editable_select(const SceneState* scene_state) {
    switch(scene_state->selected_control) {
    case HoursInput:
    case MinutesInput:
    case FontSelect:
...
bool totp_scene_app_settings_handle_event(
    ...
    if(event->input.type == InputTypePress || event->input.type == InputTypeRepeat) {
        ...
        case InputKeyUp:
            if(scene_state->editing) {
                // edit mode: encoder CCW = previous value
                change_selected_value(scene_state, -1);
                break;
            }
            totp_roll_value_uint8_t(
        ...
        case InputKeyDown:
            if(scene_state->editing) {
                // edit mode: encoder CW = next value
                change_selected_value(scene_state, 1);
                break;
            }
            totp_roll_value_uint8_t(
        ...
        case InputKeyOk:
            if(scene_state->editing) {
                // confirm and leave edit mode
                scene_state->editing = false;
                break;
            }
            if(control_is_editable_select(scene_state)) {
                // OK on a select/switch enters edit mode: rotate to change
                scene_state->editing = true;
                break;
            }
        ...
        case InputKeyBack: {
            if(scene_state->editing) {
                // leave edit mode, stay in the scene
                scene_state->editing = false;
            } else {
                totp_scene_director_activate_scene(plugin_state, TotpSceneTokenMenu);
            }

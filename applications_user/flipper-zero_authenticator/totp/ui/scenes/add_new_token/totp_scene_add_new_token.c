...
static void change_selected_value(SceneState* scene_state, int8_t direction) {
    ...
    case TokenTypeSelect:
        totp_roll_value_uint8_t(
            &scene_state->type, direction, 0, COUNT_OF(TOKEN_TYPE_LIST) - 1, RollOverflowBehaviorRoll);
        break;
    ...
    case TokenDurationOrCounterSelect:
        if(scene_state->type == TokenTypeTOTP) {
            totp_roll_value_uint8_t(
                &scene_state->duration, direction * 15, 15, UINT8_MAX, RollOverflowBehaviorStop);
...
// Whether OK on this control enters edit mode (rotate-to-change) rather than
// opening a text input or confirming.
static bool control_is_editable_select(const SceneState* scene_state) {
    ...
    case TokenLengthSelect:
        return true;
    case TokenDurationOrCounterSelect:
        return scene_state->type == TokenTypeTOTP; // HOTP counter is a text input
    default:
        return false;
    }
}
...
void totp_scene_add_new_token_render(Canvas* const canvas, const PluginState* plugin_state) {
    ...
    ui_control_select_render(
        ...
        scene_state->editing && scene_state->selected_control == TokenTypeSelect);
    ...
    ui_control_select_render(
        ...
        scene_state->editing && scene_state->selected_control == TokenAlgoSelect);
    ui_control_select_render(
        canvas,
        0,
        ...
        scene_state->editing && scene_state->selected_control == TokenLengthSelect);
    ...
    if(scene_state->type == TokenTypeTOTP) {
        ...
        scene_state->editing && scene_state->selected_control == TokenDurationOrCounterSelect);
    } else {
        ui_control_text_box_render(
            canvas,
            95 - scene_state->screen_y_offset,
    ...
    if(event->input.type == InputTypePress) {
        ...
        case InputKeyOk: {
            ...
            if(control_is_editable_select(scene_state)) {
                // OK on a select enters edit mode: rotate to change, OK to confirm

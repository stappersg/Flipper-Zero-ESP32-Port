...
bool totp_scene_generate_token_handle_event(
    ...
    if(event->input.type == InputTypeLong) {
        ...
        // T-Embed encoder has no dedicated Left/Right; bare rotation emits
        // Up/Down. Map CW (Down) -> next token, CCW (Up) -> previous token by
        // falling through to the existing Right/Left handlers.
        case InputKeyDown:
        case InputKeyRight: {
            const TokenInfoIteratorContext* iterator_context =
                totp_config_get_token_iterator_context(plugin_state);

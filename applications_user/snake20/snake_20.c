        ...
        if(event_status == FuriStatusOk) {
            if(event.type == EventTypeKey) {
                ...
                if(event.input.type == InputTypePress) {
                    ...
                    case InputKeyUp:
                        /*
                         * During pause/game-over, either wheel direction
                         * toggles Endless Mode.
                         */
                        if(snake_state->state == GameStatePause ||
                           snake_state->state == GameStateGameOver) {
                            snake_state->Endlessmode = !snake_state->Endlessmode;
                        } else {
                            /* Wheel direction 1 = turn left. */
                            switch(snake_state->currentMovement) {
                            case DirectionUp:
                    ...
                    case InputKeyLeft:
                        /*
                         * Keep Endless Mode available on actual Left/Right
                         * events, but the T-Embed wheel uses Up/Down above.
                         */
                        if(snake_state->state == GameStatePause ||
                           snake_state->state == GameStateGameOver) {
                            snake_state->Endlessmode = !snake_state->Endlessmode;

...
void tamagotchi_p1_hal_reset_clock(void);
...
static void tamagotchi_p1_draw_callback(Canvas* const canvas, void* cb_ctx) {
    ...
    if(g_ctx->rom == NULL) {
        ...
        canvas_draw_str(canvas, 37, 27, "NO ROM");

        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str(canvas, 20, 43, "tama_p1/rom.bin");
    } else if(g_ctx->halted) {
        canvas_set_font(canvas, FontPrimary);
        canvas_draw_str(canvas, 42, 29, "HALTED");

        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str(canvas, 30, 44, "Restart the app");
    } else {
        /*
         * Logical display:
         *   Canvas: 128 x 64
         *   Tama LCD: 32 x 16 pixels
         *   Scale: 2x -> 64 x 32
         */
        const uint16_t lcd_width = 32 * TAMA_SCREEN_SCALE_FACTOR;
        const uint16_t lcd_height = 16 * TAMA_SCREEN_SCALE_FACTOR;

        const uint16_t lcd_left = (128 - lcd_width) / 2;
        const uint16_t lcd_top = 16;

        /*
         * LCD frame with a small shadow.
         * Only basic canvas functions are used for FAP compatibility.
         */
        canvas_draw_frame(
            canvas,
            lcd_left - 3,
        ...
        /*
         * Draw the 32x16 Tamagotchi LCD pixels.
         */
        uint16_t y = lcd_top;
        ...
        /*
         * Status icons:
         *   Icons 0-3 above the LCD
         *   Icons 4-7 below the LCD
         */
        uint8_t lcd_icons = g_ctx->icons;

        const uint16_t icon_spacing = 17;
        const uint16_t icon_row_left = 31;
        const uint16_t upper_icon_y = 0;
        const uint16_t lower_icon_y = 50;

        for(uint8_t i = 0; i < 4; i++) {
            if(lcd_icons & (1U << i)) {
                canvas_draw_icon(
                    canvas,
                    icon_row_left + (i * icon_spacing),
                    upper_icon_y,
                    icons_list[i]);
            }
        }
        ...
        for(uint8_t i = 4; i < 8; i++) {
            if(lcd_icons & (1U << i)) {
                canvas_draw_icon(
                    canvas,
                    icon_row_left + ((i - 4) * icon_spacing),
                    lower_icon_y,
                    icons_list[i]);
            }
        }
...
int32_t tamagotchi_p1_app(void* p) {
    ...
    button_t pending_wheel_button = BTN_LEFT;
    uint8_t pending_wheel_release_ticks = 0;
    ...
    for(bool running = true; running;) {
        ...
        if(event_status == FuriStatusOk) {
            ...
            if(event.type == EventTypeTick) {
                // Release a rotary-generated Tamagotchi button after ~100 ms.
                if(pending_wheel_release_ticks > 0) {
                    pending_wheel_release_ticks--;
                    if(pending_wheel_release_ticks == 0) {
                        tamalib_set_button(
                            pending_wheel_button,
                            BTN_STATE_RELEASED);
                    }
                }
                ...
                /*
                 * T-Embed rotary clicks emit Press, Short, and Release almost
                 * immediately. Handle rotary directions on Short and hold the
                 * virtual Tamagotchi button briefly so TamaLIB can detect it.
                 */
                if(input_type == InputTypeShort) {
                    button_t tama_button;
                    bool is_tama_button = true;

                    if(event.input.key == InputKeyUp ||
                       event.input.key == InputKeyLeft) {
                        tama_button = BTN_LEFT;
                    } else if(event.input.key == InputKeyDown ||
                              event.input.key == InputKeyRight) {
                        tama_button = BTN_RIGHT;
                    } else if(event.input.key == InputKeyOk) {
                        tama_button = BTN_MIDDLE;
                    } else {
                        is_tama_button = false;
                    }

                    if(is_tama_button) {
                        // Release any previous virtual button first.
                        if(pending_wheel_release_ticks > 0) {
                            tamalib_set_button(
                                pending_wheel_button,
                                BTN_STATE_RELEASED);
                        ...
                        pending_wheel_button = tama_button;
                        pending_wheel_release_ticks = 3;
                        tamalib_set_button(
                            pending_wheel_button,
                            BTN_STATE_PRESSED);
                    }
    ...
    if(ctx->rom != NULL) {
        ...
        // The emulator is stopped, so its state can now be saved safely.
        tama_p1_save_state();
    }

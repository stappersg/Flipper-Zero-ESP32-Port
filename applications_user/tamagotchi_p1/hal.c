...
#define TAMA_CLOCK_HZ 64000ULL

static uint32_t tama_start_tick = 0;

void tamagotchi_p1_hal_reset_clock(void) {
    tama_start_tick = furi_get_tick();
}

static timestamp_t tamagotchi_p1_hal_clock_timestamp(void) {
    const uint32_t tick_frequency = furi_kernel_get_tick_frequency();
    const uint32_t elapsed_ticks = furi_get_tick() - tama_start_tick;

    return (timestamp_t)(
        ((uint64_t)elapsed_ticks * TAMA_CLOCK_HZ) / tick_frequency);
}
...
static void tamagotchi_p1_hal_sleep_until(timestamp_t ts) {
    while(true) {
        uint32_t count = tamagotchi_p1_hal_clock_timestamp();
...
static timestamp_t tamagotchi_p1_hal_get_timestamp(void) {
    return tamagotchi_p1_hal_clock_timestamp();
}

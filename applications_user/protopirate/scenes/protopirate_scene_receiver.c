...
static void protopirate_scene_receiver_update_statusbar(void* context) {
    ...
    if(app->radio_initialized && app->txrx->radio_device) {
        is_external = protopirate_radio_device_loader_is_external(app->txrx->radio_device);
    }
...
#ifndef REMOVE_LOGS
    bool is_external =
        app->txrx->radio_device ? protopirate_radio_device_loader_is_external(app->txrx->radio_device) : false;
           ...
           app->txrx->radio_device) {
            ...
            if(++rssi_log_counter >= 50) {
                ...
                bool is_external = app->txrx->radio_device ?
                                       protopirate_radio_device_loader_is_external(app->txrx->radio_device) :
                                       false;

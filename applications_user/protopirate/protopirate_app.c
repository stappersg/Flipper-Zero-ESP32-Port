...
bool protopirate_radio_init(ProtoPirateApp* app) {
    ...
    // Load keystores: plaintext KeeLoq manufacturer keys first, then encrypted Kia
    // blob last so Kia keys (types 10-13) overwrite any plaintext entries with the
    // same type. ESP32 port stubs crypto, so the encrypted load is a no-op there
    // and Kia decoders need an external supply of plaintext keys to work.
    subghz_environment_load_keystore(app->txrx->environment, PROTOPIRATE_KEYSTORE_PLAINTEXT_PATH);
    ...
    app->txrx->radio_device = protopirate_radio_device_loader_set(NULL, SubGhzRadioDeviceTypeExternalCC1101);
    ...
    if(!app->txrx->radio_device) {
        ...
        app->txrx->radio_device = protopirate_radio_device_loader_set(NULL, SubGhzRadioDeviceTypeInternal);
    }
...
void protopirate_radio_deinit(ProtoPirateApp* app) {
    ...
    if(app->txrx->radio_device) {
        ...
        protopirate_radio_device_loader_end(app->txrx->radio_device);

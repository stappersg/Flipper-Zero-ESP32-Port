...
static const uint8_t* totp_esp32_device_uid(void) {
    static uint8_t device_uid[8] = {0};
    static uint8_t initialized = 0;

    if(!initialized) {
        esp_efuse_mac_get_default(device_uid);

        /* Expand the 6-byte ESP32-S3 MAC to the 8-byte UID expected here. */
        device_uid[6] = device_uid[0] ^ device_uid[2] ^ device_uid[4];
        device_uid[7] = device_uid[1] ^ device_uid[3] ^ device_uid[5];
...
static const uint8_t* get_device_uid() {
    return totp_esp32_device_uid(); //-V566
}

...
bool totp_config_file_update_crypto_signatures(const PluginState* plugin_state) {
    ...
    flipper_format_seek_to_end(config_file);
    ...
    if(!flipper_format_write_hex(
           ...
           &plugin_state->crypto_settings.salt[0],
           CRYPTO_SALT_LENGTH)) {
        FURI_LOG_E(LOGGING_TAG, "FAIL: writing Crypto salt");
        return false;
    }
    ...
    if(!flipper_format_write_hex(
        ...
        FURI_LOG_E(LOGGING_TAG, "FAIL: writing Crypto verify");
        return false;
    }
    ...
    if(!flipper_format_write_bool(
           ...
           &plugin_state->crypto_settings.pin_required,
           1)) {
        FURI_LOG_E(LOGGING_TAG, "FAIL: writing PIN set");
        return false;
    }

    FURI_LOG_I(LOGGING_TAG, "Crypto signatures written successfully");
    return true;
}

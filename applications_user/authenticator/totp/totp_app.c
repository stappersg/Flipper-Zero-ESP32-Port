    ...
    if(dialog_result == DialogMessageButtonRight) {
        ...
        if(seed_result & CryptoSeedIVResultFlagSuccess &&
           seed_result & CryptoSeedIVResultFlagNewCryptoVerifyData) {
            /*
             * Temporary T-Embed port bypass:
             * Continue into the app even when the config-signature write fails.
             */
            if(!totp_config_file_update_crypto_signatures(plugin_state)) {
                FURI_LOG_W(LOGGING_TAG, "Config signature save failed; continuing for test");
            }

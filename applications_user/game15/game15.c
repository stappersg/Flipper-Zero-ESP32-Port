...
/*
 * T-Embed has a rotary wheel that reports Up/Down only.
 * Back short toggles the wheel between vertical and horizontal movement.
 */
static bool horizontal_mode;
...
static void render_callback(Canvas* const canvas) {
    ...
    if(game_state.scene == ScenePlay || game_state.scene == SceneWin || game_state.scene == ScenePopup) {
        ...
        canvas_set_color(canvas, ColorBlack);
        canvas_draw_str(canvas, 86, 10, horizontal_mode ? "H" : "V");
...
static void game_event_handler(GameEvent const event) {
    if(event.type == EventTypeKey) {
        if(event.input.type == InputTypePress) {
            ...
            case InputKeyUp:
                if(game_state.scene == ScenePlay && horizontal_mode) {
...
int32_t game15_app(void* context) {
    UNUSED(context);
    game_alloc();
    game_init();

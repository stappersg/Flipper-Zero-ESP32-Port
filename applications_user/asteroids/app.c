...
/*
 * Small local math helpers for the T-Embed FAP build.
 * They avoid depending on libm symbols that this firmware does not export.
 */
static float asteroids_wrap_angle(float angle) {
...
static float asteroids_sin(float angle) {
    angle = asteroids_wrap_angle(angle);
    ...
    return angle * (
        ...
        angle_sq * (
            ...
            angle_sq * (
                0.00833333f -
                angle_sq * 0.00019841f
            )
        )
    );
}

static float asteroids_cos(float angle) {
    return asteroids_sin(angle + (PI * 0.5f));
}

static float asteroids_sqrt(float value) {
...
typedef struct Ship {
    ...
    vx,                 /* x velocity. */
    vy,                 /* y velocity. */
    rot;                /* Current rotation. 2.0f * PI full rotation. */
} Ship;
...
void rotate_poly(Poly *rot, Poly *poly, float a) {
    /* We want to compute asteroids_sin(a) and asteroids_cos(a) only one time
     * for every point to rotate. It's a slow operation. */
    float sin_a = asteroids_sin(a);
    float cos_a = asteroids_cos(a);
...
void draw_asteroid(Canvas *const canvas, Asteroid *ast) {
     ...
     * asteroids_sin() / asteroids_cos(). But the Flipper can handle it without problems. */
    ...
    for (int j = 0; j < 8; j++) {
        ...
        lfsr_next(&r);
        float scaling = 0.7f + ((float)r / 255.0f * 0.6f);

        ap.x[j] = asteroids_sin(a) * ast->size * scaling;
        ap.y[j] = asteroids_cos(a) * ast->size * scaling;
    }
    ap.points = 8;
...
float distance(float x1, float y1, float x2, float y2) {
    float dx = x1-x2;
    float dy = y1-y2;
    return asteroids_sqrt(dx*dx+dy*dy);
}
                           ...
                           float factor)
{
     ...
     * So it would be like: asteroids_sqrt((x1-x2)^2+(y1-y2)^2) < r1+r2.
...
void ship_fire_bullet(AsteroidsApp *app) {
    ...
    b->x = app->ship.x;
    b->y = app->ship.y;
    b->vx = -asteroids_sin(app->ship.rot);
    b->vy = asteroids_cos(app->ship.rot);
...
Asteroid *add_asteroid(AsteroidsApp *app) {
    ...
    a->x = x;
    a->y = y;
    a->vx = 2.0f * (-0.5f + ((float)rand() / (float)RAND_MAX));
    a->vy = 2.0f * (-0.5f + ((float)rand() / (float)RAND_MAX));
    a->size = size;
    a->rot = 0;
    a->rot_speed = ((float)rand() / (float)RAND_MAX) / 10.0f;
...
void update_asteroids_position(AsteroidsApp *app) {
    for (int j = 0; j < app->asteroids_num; j++) {
        ...
        if (app->asteroids[j].rot < 0) app->asteroids[j].rot = 2.0f * PI;
        else if (app->asteroids[j].rot > 2.0f * PI) app->asteroids[j].rot = 0;
    }
}
...
void game_tick(void *ctx) {
    ...
    /* Handle key presses. */
    if (app->pressed[InputKeyLeft]) app->ship.rot -= 0.35f;
    if (app->pressed[InputKeyRight]) app->ship.rot += 0.35f;
    if (key_pressed_time(app,InputKeyOk) > SHIP_ACCELERATION_KEYPRESS_TIME) {
        app->ship.vx -= 0.5f * asteroids_sin(app->ship.rot);
        app->ship.vy += 0.5f * asteroids_cos(app->ship.rot);
...
void asteroids_update_keypress_state(AsteroidsApp *app, InputEvent input) {
    if(input.type == InputTypePress) {
        /*
         * The T-Embed wheel reports quick Up/Down events, not held
         * Left/Right presses. Rotate immediately for every wheel step.
         */
        ...
        if(dur < 200 && input.key == InputKeyOk) {
            app->fire = true;
        }
    }
}

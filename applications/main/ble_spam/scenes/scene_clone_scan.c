...
void ble_spam_scene_clone_scan_on_enter(void* context) {
...
bool ble_spam_scene_clone_scan_on_event(void* context, SceneManagerEvent event) {
    BleSpamApp* app = context;
    bool consumed = false;
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == InputKeyUp) {
            ...
            view_commit_model(app->view_walk_scan, false);

            if(dev_count > 0) {
                app->clone_selected_device = selected;
                // Stop scan, stop walk HAL (we'll use spam HAL for cloning)
                ble_walk_hal_stop_scan();
                ble_walk_hal_stop();
                scene_manager_next_scene(app->scene_manager, BleSpamSceneCloneActive);
            }
            consumed = true;
        }
...
void ble_spam_scene_clone_scan_on_exit(void* context) {
    UNUSED(context);
}

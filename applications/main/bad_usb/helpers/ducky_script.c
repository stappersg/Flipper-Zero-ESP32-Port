...
static bool ducky_script_preload(BadUsbScript* bad_usb, File* script_file) {
    ...
    if(bad_usb->load_id_cfg) {
        ...
        if(strncmp(line_tmp, ducky_cmd_id, strlen(ducky_cmd_id)) == 0) {
            if(ducky_set_usb_id(bad_usb, &line_tmp[strlen(ducky_cmd_id) + 1])) {
                if(!bad_usb_hid_supports_usb()) {
                    FURI_LOG_W(
                        WORKER_TAG,
                        "USB HID unsupported on this target, keeping current BadUSB interface");
                } else if(furi_hal_usb_is_locked()) {
                    FURI_LOG_W(WORKER_TAG, "USB is locked, keeping current BadUSB interface");
                } else {
        ...
        if((interface == BadUsbHidInterfaceUsb) && !bad_usb_hid_supports_usb()) {
            interface = BadUsbHidInterfaceBle;
        }
...
BadUsbScript* bad_usb_script_open(
    ...
    bad_usb->thread = furi_thread_alloc_ex("BadUsbWorker", 4096, bad_usb_worker, bad_usb);
